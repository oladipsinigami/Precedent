...
export function Header({ onLoadDemo }: HeaderProps) {
  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#07090c]/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-[1520px] items-center justify-between px-3.5 py-2.5 sm:px-5 sm:py-3.5 lg:px-8">
        <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-sm bg-[#d4ff3f] font-mono text-xs font-black text-[#080a0d] shadow-[0_0_16px_rgba(212,255,63,0.35)]">
            P
          </div>
          <div className="min-w-0">
            ...
            <div className="hidden sm:block text-[11px] text-[#71808e] truncate">
        ...
        <div className="flex items-center gap-2 sm:gap-5 font-mono text-[11px] uppercase tracking-[0.14em] shrink-0">
          <span className="hidden items-center gap-1.5 text-[#62707d] lg:inline-flex">
            <span className="h-1 w-1 rounded-full bg-[#52606d]" />
            Non-Execution Environment
          </span>

          <div className="flex items-center gap-1.5 sm:gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 sm:px-3 sm:py-1 text-[#a0b0be]">
            ...
            <span className="text-[9px] sm:text-[10px] tracking-[0.12em]">
              <span className="hidden sm:inline">Bitget </span>Live Tape
            </span>
          </div>
          ...
          <button
            type="button"
            onClick={onLoadDemo}
            className="group flex min-h-[44px] items-center gap-1.5 px-1 sm:px-0 text-xs text-[#d4ff3f] transition hover:text-white"
